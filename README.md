...
Данные сохраняются локально в системной папке пользователя:

- Windows: `%APPDATA%\AutoPiar`
- Linux/Termux: `~/.autopiar`
...
После успешной проверки данные сохранятся локально в системной папке пользователя, а не рядом с программой.
...
## Как отдавать клиентам чистую сборку

Клиентам не нужно скачивать весь репозиторий. Для ПК отдавайте только архив из папки `release/`:

```text
release/AutoPiar.zip
```

Внутри архива должен быть только `AutoPiar.exe` и короткая инструкция. Все служебные данные приложение создаёт само в `%APPDATA%\AutoPiar`.

Так пользователи не видят:
